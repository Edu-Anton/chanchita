...
@section('content')
    <div class="d-flex justify-content-between">
      <h3 class="title1 mb-4">Modificar Chanchita</h3>
      <a href="{{route('chanchita.show', $chanchita)}}" class="btn btn-link">Regresar</a>
    </div>
    <div class="card">
      <div class="card-body">
        @if (session('msg'))
        ...
        @endif
        <form action="{{route('chanchita.update', [$chanchita])}}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PATCH')
          ...
          <div class="text-center mb-4">
            <div class="mb-4">
              <img class="card-image-form__img" src="{{ url(Storage::url($chanchita->url_img)) }}" alt="" id="imagen_mostrada">
              {{-- <img class="img-vacia__icon mr-2" src="{{ asset('img/camera.svg') }}" alt="Juguetes"> --}}
              {{-- Formato de imagen 3:4 --}}
            </div>
            <input 
                type="file" 
                class="form-control @error('url_img') is-invalid @enderror d-none" 
                id="product-file" 
                name="url_img"
                value="{{$chanchita->url_img}}"
            >
  ...
  >{{$chanchita->description}}"
          ...
          <div class="row mt-4">
            <div class="offset-3 col-9">
              <button class="btn button__base button__pink" type="submit">Actualizar información</button>
            </div>
          </div>
        </form>
      </div>
